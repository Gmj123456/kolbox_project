...
public class JwtUtil {
	 ...
	 * @return 加密的token
	 */
	public static String sign(String username, String id, String realname, String secret) {
		Date date = new Date(System.currentTimeMillis() + EXPIRE_TIME);
		Algorithm algorithm = Algorithm.HMAC256(secret);
		// 附带username信息
		return JWT.create().withClaim("username", username).withClaim("id", id).withClaim("realname", realname).withExpiresAt(date).sign(algorithm);
	}
	 ...
	 * @throws JeecgBootException
	 */
	public static String getRealNameByToken(HttpServletRequest request) throws JeecgBootException {
		String accessToken = request.getHeader("X-Access-Token");
		String realname = "";
		if (oConvertUtils.isNotEmpty(accessToken)) {
			realname = getRealname(accessToken);
		}
       /* if (oConvertUtils.isEmpty(username)) {
            throw new JeecgBootException("未获取到用户");
        }*/
