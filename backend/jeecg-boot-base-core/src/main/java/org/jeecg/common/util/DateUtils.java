...
public class DateUtils extends PropertyEditorSupport {
    ...
    public static String date2Str(Date date1) {
        SimpleDateFormat sformat = new SimpleDateFormat(formatStyle);
    ...
    public static Date getFetureDate(int past) {
        ...
        Date parse = null;
        try {
            parse = dateFormat.parse(format.format(today));
        } catch (Exception e) {
            e.printStackTrace();
        }
        return parse;
     ...
     * @return
     */
    public static Date getFetureDateMinute(Date date, int past) {
        Calendar calendar = Calendar.getInstance();
     ...
     * 获取系统时间前12小时
     */
    public static String getNewTime12Date() {
     ...
     * 获取系统几天之后的时间
     */
    public static String getAfterByDayTime(int day) {
        Calendar beforeTime = Calendar.getInstance();
     ...
     * 获取系统几天之前的时间
     */
    public static Date getBeforeByDayTime(int day) {
        Calendar beforeTime = Calendar.getInstance();
        beforeTime.add(Calendar.DAY_OF_MONTH, -day);
     ...
     * 获取具体某个时间
     */
    public static String getSpecificTime(int dateNum) {
        Date date = new Date();
        date.setDate(date.getDate());
        date.setHours(dateNum);
        date.setMinutes(0);
        date.setSeconds(0);
        String s = DateUtils.formatDate(date, formatStyle);
        return s;
    }
     ...
     * 获取具体某个时间
     */
    public static Date getSpecificTime2(int dateNum) {
        Date date = new Date();
        date.setDate(date.getDate());
        date.setHours(dateNum);
        date.setMinutes(0);
    ...
    public static String getDayEnd(Date date) {
        ...
        String s = formatDate(endOfDate, "yyyy-MM-dd");
        return s;
    }
     ...
     * @return
     */
    public static Date getFetureMOnthDate(int past, Date date) {
        Calendar calendar = Calendar.getInstance();
        calendar.setTime(date);
     ...
     * @return
     */
    public static Date getFetureDateHour(int past, Date date) {
        Calendar calendar = Calendar.getInstance();
        calendar.setTime(date);
        calendar.add(Calendar.HOUR, past);
        return calendar.getTime();
    }
     ...
     * @return
     */
    public static Date getFetureDays(int past) {
        Calendar calendar = Calendar.getInstance();
        calendar.add(Calendar.DATE, past);
        return calendar.getTime();
    }
     ...
     *
     * @Author: nqr
     * @Date 2020-12-03 17:47:47
     */

    public static Date getDateTime(Date date, int hour) {
     ...
     *
     * @Author: nqr
     * @Date 2020-12-15 13:43:19
     */
    public static Date getDatePoint(Date date, int num) {
    ...
    /**
     * 功能描述:获取当前时间前num个月的时间
     **/

    public static Date getMonthsAgoDate(int num) {
        LocalDateTime sixMonthsAgo = LocalDateTime.now().minusMonths(num);
        return Date.from(sixMonthsAgo.atZone(java.time.ZoneId.systemDefault()).toInstant());
    }

    public static Date str2dateS(String src) {
        Date parse = null;
        try {
            parse = getSDFormat("yyyy-MM-dd").parse(src);
        } catch (Exception e) {
     ...
     * @return
     */
    public static String dateStrAddOneDay(String dateStr) {
        SimpleDateFormat sdf = new SimpleDateFormat("yyyy-MM-dd");
        Date dateAfter = null;
        try {
            dateAfter = sdf.parse(dateStr);
            Calendar calendar = new GregorianCalendar();
        ...
        // 获取String类型的时间
        String enddateStr = sdf.format(dateAfter);
        return enddateStr;
    }
    /**
     * 日期转换为字符串
