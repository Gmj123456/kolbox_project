...
//	}
	...
	public static <T> List<List<T>> splitListIntoBatches(List<T> originalList, int batchSize) {
		List<List<T>> batches = new ArrayList<>();

		if (originalList == null || originalList.isEmpty()) {
			return batches; // 返回空列表
		}
	...
	/**
	 * 将对象中所有 String 类型的空字符串（包括空白字符）字段设为 null
	 */
	public static void convertEmptyToNull(Object obj) {
		if (obj == null) return;
	...
	public static String formatLargeNumber(int number) {
		if (number < 1000) {
			...
			return String.format("%.0fK", number / 1000.0);
	...
	/**
	 * 支持小数金额的格式化（解决 35512.07 → 35.5K 的问题）
	 */
	public static String formatLargeNumber(double num) {
		if (num < 1000) {
			...
			double k = num / 1000.0;
			String str = String.format("%.1fK", k);
			return str.endsWith(".0K") ? str.replace(".0K", "K") : str;
		} else {
			double m = num / 1_000_000.0;
			String str = String.format("%.1fM", m);
			return str.endsWith(".0M") ? str.replace(".0M", "M") : str;
		}
	}

	/**
	 * 判断该对象是否为null: 返回ture表示所有属性为null  返回false表示不是所有属性都是null
	 *
	 * @param obj
	 * @return
	...
	/**
	 * 判断当前用户是否是管理员：返回true 表示是，返回false 表示不是
	 */
	public static boolean checkUserType(String username) {
		return "admin".equals(username) || "ptuser".equals(username);
	}
	/**
	 * 校验邮箱格式是否正确
	 *
