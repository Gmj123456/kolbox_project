...
public class RestUtil {
    ...
    public static <T>ResponseEntity<T> postNative(String url, JSONObject variables, JSONObject params, Class<T> targetType) {
        ResponseEntity<JSONObject> request = request(url, HttpMethod.POST, variables, params);
        return convertResponse(request, targetType);
    }
    /**
     * 通用响应转换方法
     ...
     * @param <T> 目标类型
     * @return 转换后的ResponseEntity
     */
    public static <T> ResponseEntity<T> convertResponse(ResponseEntity<?> originalResponse, Class<T> targetType) {
        try {
            T responseBody = convertBody(originalResponse.getBody(), targetType);
            ...
            throw new RuntimeException("响应转换失败: " + e.getMessage(), e);
        }
    }
    /**
     * 转换响应体
     */
    private static <T> T convertBody(Object body, Class<T> targetType) throws Exception {
        if (body == null) {
            return null;
        }
        ...
        if (body instanceof JSONObject) {
            return objectMapper.readValue(body.toString(), targetType);
        } else if (body instanceof JsonNode) {
            return objectMapper.treeToValue((JsonNode) body, targetType);
        } else if (body instanceof String) {
            return objectMapper.readValue((String) body, targetType);
        } else {
            return objectMapper.convertValue(body, targetType);
        }
    }
    /**
     * 发送 put 请求
