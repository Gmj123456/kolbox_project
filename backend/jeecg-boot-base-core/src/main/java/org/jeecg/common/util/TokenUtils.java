...
public class TokenUtils {
    ...
    public static boolean verifyToken(String token, CommonAPI commonApi, RedisUtil redisUtil) {
        ...
        if (!jwtTokenRefresh(token, username, user.getId(), user.getRealname(), user.getPassword(), redisUtil)) {
     ...
     * @return
     */
    private static boolean jwtTokenRefresh(String token, String userName, String id, String realname, String passWord, RedisUtil redisUtil) {
        String cacheToken = oConvertUtils.getString(redisUtil.get(CommonConstant.PREFIX_USER_TOKEN + token));
        if (oConvertUtils.isNotEmpty(cacheToken)) {
            // 校验token有效性
            if (!JwtUtil.verify(cacheToken, userName, passWord)) {
                String newAuthorization = JwtUtil.sign(userName, id, realname, passWord);
                // 设置Toekn缓存有效时间
