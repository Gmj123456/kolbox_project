 ...
 * @Version: V1.0
 */
public interface KolAttributeMapper extends BaseMapper<KolAttribute> {
     ...
     * @return 类目路径列表
     */
    List<KolAttribute> findCategoryWithSubNodes(@Param("categoryId") String categoryId);

    List<KolAttributeModel> getAttributeList(@Param("searchModel") KolSearchModel searchModel);

    List<String> queryAttributeIdsByProductId(@Param("productId") String productId,@Param("platformType") Integer platformType);

    List<String> queryProductAttributes(@Param("productId")String productId);
}
