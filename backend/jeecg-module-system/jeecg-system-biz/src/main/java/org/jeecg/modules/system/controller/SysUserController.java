...
import org.jeecg.modules.kol.model.KolCounselorModel;
...
public class SysUserController extends JeecgController<SysUser, ISysUserService> {
    ...
    @Autowired
    private IStoreUserCounselorService userCounselorService;
    ...
    public Result<JSONObject> userRegister(@RequestBody JSONObject jsonObject, SysUser user) {
        ...
        // String username =
        // Optional.ofNullable(jsonObject.get("username")).orElse(email).toString();
        String username = jsonObject.getString("username");

        boolean hasPhone = oConvertUtils.isNotEmpty(phone);
        boolean hasEmail = oConvertUtils.isNotEmpty(email);

        if (!hasPhone && !hasEmail) {
            result.setMessage("手机号和邮箱至少需要填写一个");
            result.setSuccess(false);
            return result;
        }
        ...
        if (hasPhone) {
            Object smsCodeObj = redisUtil.get(CommonConstant.PHONE_REDIS_KEY_PRE + phone);
            if (null == smsCodeObj) {
                result.setMessage("手机验证码失效！");
                result.setSuccess(false);
                return result;
            }
            String redisSmsCode = "";
            if (smsCodeObj.toString().contains("code")) {
                redisSmsCode = JSONObject.parseObject(smsCodeObj.toString()).getString("code");
            } else {
                redisSmsCode = smsCodeObj.toString();
            }
        ...
        // 未设置用户名，则用手机号或邮箱作为用户名
        if (oConvertUtils.isEmpty(username)) {
            username = hasPhone ? phone : email;
        }
        // 未设置密码，则随机生成一个密码
        ...
        if (sysUser1 != null) {
            Integer delFlag = sysUser1.getDelFlag();
            Integer status = sysUser1.getStatus();
            result.setMessage(delFlag == 1 || status == 2 ? "用户状态异常，无法注册" : "用户名已存在");
            result.setSuccess(false);
            return result;
        }
        ...
        if (sysUser2 != null) {
            Integer delFlag = sysUser1.getDelFlag();
            Integer status = sysUser1.getStatus();
            result.setMessage(delFlag == 1 || status == 2 ? "用户状态异常，无法注册" : "该手机号已注册");
            result.setSuccess(false);
            return result;
        }
        ...
        if (oConvertUtils.isNotEmpty(email)) {
            ...
            if (sysUser3 != null) {
                Integer delFlag = sysUser1.getDelFlag();
                Integer status = sysUser1.getStatus();
                result.setMessage(delFlag == 1 || status == 2 ? "用户状态异常，无法注册" : "邮箱已被注册");
                result.setSuccess(false);
                return result;
            }
        }
       ...
       /* try {
            SysDictItem dictItem = dictItemService.selectValueByCode(DictCode.COUNSELOR_USER);
            if (oConvertUtils.isNotEmpty(dictItem)) {
                SysUser sysUser = sysUserService.getById(dictItem.getItemValue());
                List<StoreUserCounselor> list = storeUserCounselorService.lambdaQuery().eq(StoreUserCounselor::getUsername, sysUser.getUsername()).list();
                user.setSellerCounselorId(dictItem.getItemValue());
                user.setSellerCounselorName(sysUser.getUsername());
                if (list != null && !list.isEmpty()) {
                    String userId = list.get(0).getSysUserId();
                    SysUser counselorUser = sysUserService.getById(userId);
                    user.setCounselorUserId(list.get(0).getSysUserId());
                    user.setCounselorUserName(counselorUser.getUsername());
                }
                user.setCounselorUserId(sysUser.getCounselorUserId());
                user.setCounselorUserName(sysUser.getCounselorUserName());
            }
        } catch (Exception e) {
            log.error("查询默认商家顾问失败", e);
        }*/
        ...
        try {
            ...
            user.setUserType(UserType.SELLER.getCode());
            sysUserService.addUserWithRole(user, "1311196333680357378");// 默认临时角色 test
            result.success("注册成功");
        } catch (Exception e) {
...
//	}
     ...
     * @Author: zhoushen
     */
    @AutoLog(value = "查询所有网红顾问-查询")
    @GetMapping(value = "/queryAllCelebrityCounselor")
    public Result<?> queryAllCelebrityCounselor() {
        // 2025年9月18日09:19:14 刘工说增加是否离职查询条件，不查询已离职的顾问
        LambdaQueryWrapper<SysUser> lambdaQueryWrapper = new LambdaQueryWrapper<>();
        lambdaQueryWrapper.eq(SysUser::getDelFlag, YesNoStatus.NO.getCode())
                .eq(SysUser::getIsLeave, 0)
                .eq(SysUser::getUserType, UserType.CELEBRITY_COUNSELOR.getCode());
//        return Result.ok(sysUserService.list(lambdaQueryWrapper));
        // 使用select方法指定要查询的列
        lambdaQueryWrapper.select(SysUser::getId, SysUser::getUsername, SysUser::getShortName);
        List<SysUser> userList = sysUserService.list(lambdaQueryWrapper);
        // 将user类转换为KolCounselorModel 将其他无用字段移除
        List<KolCounselorModel> counselorModelList = userList.stream().map(user -> {
            KolCounselorModel counselorModel = new KolCounselorModel();
            counselorModel.setId(user.getId());
            counselorModel.setUsername(user.getUsername());
            counselorModel.setRealname(user.getRealname());
            counselorModel.setShortName(user.getShortName());
            return counselorModel;
        }).collect(Collectors.toList());

        return Result.ok(counselorModelList);
    }
     ...
     * @Date 2021-07-06 14:45:26
     */
    @AutoLog(value = "获取商务人员-查询")
    @GetMapping(value = "/getBusinessPeople")
    public Result<?> getBusinessPeople() {
        LambdaQueryWrapper<SysUser> queryWrapper = new LambdaQueryWrapper<>();
        queryWrapper.eq(SysUser::getUserType, UserType.BUSINESS_PERSONNEL.getCode());
        queryWrapper.eq(SysUser::getIsLeave, YesNoStatus.NO.getCode());
        queryWrapper.eq(SysUser::getStatus, YesNoStatus.YES.getCode());
        queryWrapper.eq(SysUser::getDelFlag, YesNoStatus.NO.getCode());
     ...
     * @return
     * @Author: nqr
     * @Date 2021-03-24 09:54:56
     */
    @RequestMapping(value = "/getSellerCounselor", method = RequestMethod.GET)
    public Result<?> getSellerCounselor() {
        LambdaQueryWrapper<SysUser> lambdaQueryWrapper = new LambdaQueryWrapper<>();
        lambdaQueryWrapper.eq(SysUser::getUserType, UserType.BUSINESS_COUNSELOR.getCode());
        lambdaQueryWrapper.isNotNull(SysUser::getNickname);
        return Result.ok(sysUserService.list(lambdaQueryWrapper));
    }


    @RequestMapping(value = "/getSellerById", method = RequestMethod.GET)
    public Result<?> getSellerById(@RequestParam(name = "id") String id) {
        return Result.ok(sysUserService.lambdaQuery().eq(SysUser::getId, id).one());
    }
    ...
    public Result<?> getSellerList(@RequestParam(name = "sellerCounselorId", required = true) String sellerCounselorId) {
        LambdaQueryWrapper<SysUser> lambdaQueryWrapper = new LambdaQueryWrapper<>();
        lambdaQueryWrapper.eq(SysUser::getSellerCounselorId, sellerCounselorId);
        return Result.ok(sysUserService.list(lambdaQueryWrapper));
    }

    @RequestMapping(value = "/getSellerCounselorById", method = RequestMethod.GET)
    public Result<?> getSellerCounselorById(@RequestParam(name = "id") String id) {
        LambdaQueryWrapper<SysUser> lambdaQueryWrapper = new LambdaQueryWrapper<>();
        lambdaQueryWrapper.eq(SysUser::getUserType, UserType.BUSINESS_COUNSELOR.getCode());
        lambdaQueryWrapper.isNotNull(SysUser::getNickname);
        lambdaQueryWrapper.eq(SysUser::getCounselorUserId, id);
        return Result.ok(sysUserService.list(lambdaQueryWrapper));
    }
    ...
    public Result<JSONObject> addSeller(@RequestBody JSONObject jsonObject, SysUser user) {

        Result<JSONObject> result = new Result<>();
   /*     // 查询有无默认商家等级
        LambdaQueryWrapper<StoreSellerGrade> lambdaQueryWrapper = new LambdaQueryWrapper<>();
        lambdaQueryWrapper.eq(StoreSellerGrade::getIsDefault, 1);

        StoreSellerGrade sellergrade = storeSellerGradeService.getOne(lambdaQueryWrapper);
        if (oConvertUtils.isEmpty(sellergrade)) {
            return result.error500("没有默认商家等级，请联系管理员！");
        }*/

        String phone = jsonObject.getString("phone");
        String email = jsonObject.getString("email");
        String username = jsonObject.getString("username");
        String sellerCounselorId = jsonObject.getString("sellerCounselorId");
        String sellerCounselorName = jsonObject.getString("sellerCounselorName");
        String counselorUserId = jsonObject.getString("counselorUserId");
        String counselorUserName = jsonObject.getString("counselorUserName");
        String companyName = jsonObject.getString("companyName");
        String feishuAccount = jsonObject.getString("feishuAccount");
        String acquisitionChannel = jsonObject.getString("acquisitionChannel");
        String remark = jsonObject.getString("remark");
        if (oConvertUtils.isEmpty(sellerCounselorName)) {
            StoreUserCounselor storeUserCounselor = userCounselorService.getById(sellerCounselorId);
            if (storeUserCounselor != null) {
                sellerCounselorName = storeUserCounselor.getUsername();
            }
        }
        String wxAccount = jsonObject.getString("wxAccount");
        if (oConvertUtils.isEmpty(sellerCounselorId)) {
            result.setMessage("请选择商家顾问");
            result.setSuccess(false);
            return result;
        }
        // 未设置密码，则随机生成一个密码
      ...
      /*  try {
        ...
        StoreEmailPackagePurchase storeEmailPackagePurchase = new StoreEmailPackagePurchase();
        ...
        try {
            ...
            user.setStatus(1);
            user.setPhone(phone);
            user.setStatus(YesNoStatus.YES.getCode());
            user.setDelFlag(CommonConstant.DEL_FLAG_0);
            user.setActivitiSync(CommonConstant.ACT_SYNC_1);
/*            // 设置卖家等级
            user.setGradeId(sellergrade.getId());
            // 设置卖家等级
            user.setGradeName(sellergrade.getGradeName());
            // 设置每日额度，邮箱个数，套餐个数
            user.setEmailsDay(sellergrade.getEmailPushCount());
            user.setEmailAccountCount(sellergrade.getEmailAccountCount());
            user.setEmailPushCount(sellergrade.getEmailPushCount());*/
            // 设置随机邀请码
            ...
            user.setCounselorUserId(oConvertUtils.isEmpty(counselorUserId)?getUserIdByToken():counselorUserId);
            user.setCounselorUserName(oConvertUtils.isEmpty(counselorUserName)?getRealName():counselorUserName);
            user.setWxAccount(wxAccount);
            user.setCompanyName(companyName);
            user.setFeishuAccount(feishuAccount);
            user.setAcquisitionChannel(acquisitionChannel);
            user.setRemark(remark);
            // 获取到期时间 默认普通会员等级周期为天数
            // user.setExpirationDate(DateUtils.getFetureDate(sellergrade.getGradeCycle()));
            user.setDepartIds("e6606c64fd9643d182fb0a504e2f6e6c");
            user.setApprovedStatus(YesNoStatus.NO.getCode());
            sysUserService.addSellerUser(user, "1311196333680357378", "e6606c64fd9643d182fb0a504e2f6e6c", storeEmailPackagePurchase);
            result.success("新增成功");
        } catch (Exception e) {
    ...
    private String getInvitationCode() {
        String invitationCode = oConvertUtils.randomGen(8);
        // 查询用户表中是否已经存在此邀请码
        LambdaQueryWrapper<SysUser> lambdaSysUserQueryWrapper = new LambdaQueryWrapper<>();
        lambdaSysUserQueryWrapper.eq(SysUser::getInvitationCode, invitationCode);
        List<SysUser> sysUsers = sysUserService.list(lambdaSysUserQueryWrapper);
        if (sysUsers.size() > 0) {
            getInvitationCode();
        }
    ...
    public Result<SysUser> userPasswordChange(@RequestParam(name = "username") String username, @RequestParam(name = "password") String password, @RequestParam(name = "smsCode") String smsCode, @RequestParam(name = "email") String email) {
        ...
        Object email_code = redisUtil.get(email);
        String emailStr = oConvertUtils.isNotEmpty(email_code) ? email_code.toString() : null;
        if (oConvertUtils.isEmpty(email_code)) {
            result.setMessage("邮箱验证码失效！");
            result.setSuccess(false);
            return result;
        }
        if (!smsCode.equals(emailStr)) {
            result.setMessage("邮箱验证码不匹配！");
            result.setSuccess(false);
            return result;
        }
        SysUser sysUser = this.sysUserService.getOne(new LambdaQueryWrapper<SysUser>().eq(SysUser::getUsername, username).eq(SysUser::getEmail, email));
    ...
    public Result<JSONObject> wxRegister(@RequestBody SysUser user) {
        Result<JSONObject> result = new Result<>();
        ...
        try {
            ...
            return result.error500("请检查用户名是否含有特殊字符！");
        }
        if (sysUser != null) {
            result.setMessage("用户名已注册");
            result.setSuccess(false);
...
//            user.setExpirationDate(DateUtils.getFetureDate(sellergrade.getGradeCycle()));
            user.setDepartIds("e6606c64fd9643d182fb0a504e2f6e6c");
            user.setApprovedStatus(YesNoStatus.NO.getCode());
            sysUserService.addSellerUser(user, "1311196333680357378", "e6606c64fd9643d182fb0a504e2f6e6c", new StoreEmailPackagePurchase());
            // 生成token
            String token = JwtUtil.sign(user.getUsername(), user.getId(), user.getRealname(), user.getPassword());
            // 设置token缓存有效时间
            redisUtil.set(CommonConstant.PREFIX_USER_TOKEN + token, token);
            redisUtil.expire(CommonConstant.PREFIX_USER_TOKEN + token, JwtUtil.EXPIRE_TIME * 2 / 1000);
            resJson.put("token", token);
            resJson.put("userInfo", user);
            result.success("注册成功");
        } catch (Exception e) {
            result.error500("注册失败");
        }
     ...
     * @return
     */
    @AutoLog(value = "设置商家顾问")
    @PostMapping("/editSeller")
    public Result<?> editSeller(@RequestBody JSONObject jsonObject) {
