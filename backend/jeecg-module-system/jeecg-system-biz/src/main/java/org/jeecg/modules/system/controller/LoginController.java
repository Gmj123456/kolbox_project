...
import org.jeecg.modules.system.config.LoginConfig;
...
import org.jeecg.modules.system.util.MailUtils;
...
public class LoginController {
	...
	public Result<JSONObject> login(@RequestBody SysLoginModel sysLoginModel, HttpServletRequest request){
		...
		String email = sysLoginModel.getEmail();
		String mobile = sysLoginModel.getMobile();
		if (oConvertUtils.isEmpty(email) && oConvertUtils.isEmpty(username) && oConvertUtils.isEmpty(mobile)){
			return result.error500("用户名、邮箱或手机号不能为空");
		}
		...
		}else if(oConvertUtils.isNotEmpty(email)){
			queryWrapper.eq(SysUser::getEmail,email);
		}else if(oConvertUtils.isNotEmpty(mobile)){
	...
	public Result<String> sms(@RequestBody JSONObject jsonObject,HttpServletRequest request) {
		...
		log.info("-------- IP:{}, 手机号：{}，验证码：{}", clientIp, mobile, captcha);
		JSONObject obj = new JSONObject();
	...
	private Result<JSONObject> userInfo(SysUser sysUser, Result<JSONObject> result, HttpServletRequest request) {
		...
		//1.生成token
		String token = JwtUtil.sign(username, user.getId(), user.getRealname(), syspassword);
		// 设置token缓存有效时间
	...
	public Result<?> getEmailCode(@RequestBody JSONObject jsonObject) {
		Result<?> result = new Result<>();
		String email = jsonObject.get("email").toString();
//		String username = Optional.ofNullable(jsonObject.get("username")).orElse(email).toString();
		String username = jsonObject.getString("username");
		String smsmode = jsonObject.get("smsmode").toString();
		String code = oConvertUtils.randomGen(5);

		boolean validEmail = oConvertUtils.isValidEmail(email);
		if (!validEmail) {
		...
		SysUser sysUser;
		if (!"2".equals(smsmode)) {
			sysUser = this.sysUserService.getOne(new LambdaQueryWrapper<SysUser>().eq(SysUser::getEmail, email));
			if (sysUser == null) {
				System.err.println("emailCode >>>>>>> " + code);
				//存储验证码，有效期5min
				redisUtil.set(email, code, 300);
				new Thread(() -> MailUtils.toMail(email, username, code, 0)).start();
				//MailUtils.toMail(email, code);
				result.setSuccess(true);
				result.setCode(200);
				return result;
			} else {
				return Result.error("邮箱已存在,请直接登录！");
			}
		} else {
			System.err.println("emailCode >>>>>>> " + code);
			//存储验证码，有效期5min
	 ...
	 *
	 * @Author: nqr
	 * @Date 2021-03-26 10:07:26
	 */
	@GetMapping(value = "/qqLogin")
	public Result<JSONObject> qqLogin(@RequestParam(name = "code", required = true) String code) {
		Result<JSONObject> result = new Result<>();
		JSONObject resJson = new JSONObject();
		//获取access_token
		String url = "https://graph.qq.com/oauth2.0/token?grant_type=authorization_code&client_id=" + LoginConfig.QQAPPID + "&client_secret=" + LoginConfig.QQSECRET + "&code=" + code + "&redirect_uri=https%3a%2f%2fp.kolbox.com%2fuser%2fqq-loading";
		String data = ToolHttpClient.httpGet(url, new HashMap<>(), null);
		...
		String accessToken = data.split("&")[0].split("=")[1];
		//获取openId
		String openIdUrl = "https://graph.qq.com/oauth2.0/me?access_token=" + accessToken;
		String openIdData = ToolHttpClient.httpGet(openIdUrl, new HashMap<>(), null);
		System.out.println("获取openId----------------------------------------------------------");
		System.out.println(openIdData);
		openIdData = openIdData.replace("callback( ", "").replace(" );", "");
		JSONObject json = JSONObject.parseObject(openIdData);
		String openid = json.getString("openid");
		//获取用户信息
		String userInfoUrl = "https://graph.qq.com/user/get_user_info?access_token=" + accessToken + "&oauth_consumer_key=101941012&openid=" + openid;
		String userInfoData = ToolHttpClient.httpGet(userInfoUrl, new HashMap<>(), null);
		JSONObject userJson = JSONObject.parseObject(userInfoData);
		String nickName = userJson.getString("nickname");
		String sex = userJson.getString("gender_type");
		String headimgUrl = userJson.getString("figureurl_qq");
//         if (oConvertUtils.isNotEmpty(json.get("errcode"))) {
//             result.setSuccess(false);
//             result.setCode(500);
//             result.setResult(json);
//             result.setMessage("无效的请求！");
//             return result;
//         }
		//判断当前用户是否存在
		SysUser sysUser = checkUser(openid);
		//用户存在
		if (oConvertUtils.isNotEmpty(sysUser)) {
			SysUser userNew = new SysUser();
			userNew.setId(sysUser.getId());
			//判断昵称头像是否一致，不一致更新
			if (!nickName.equals(sysUser.getNickname())) {
				sysUser.setNickname(nickName);
				userNew.setNickname(nickName);
			}
			if (!headimgUrl.equals(sysUser.getAvatar())) {
				sysUser.setAvatar(headimgUrl);
				userNew.setAvatar(headimgUrl);
			}
			sysUser.setUnionId(openid);
			//修改用户信息
			sysUserService.updateById(userNew);
			// 生成token
			String token = getToken(sysUser);
			//用户登录信息
			resJson.put("token", token);
			resJson.put("userInfo", sysUser);
		} else {
			//用户不存在，返回已知信息，等待前端提交
	 ...
	 *
	 * @Author: nqr
	 * @Date 2021-03-10 16:32:31
	 */
	@GetMapping(value = "/wxLogin")
	public Result<JSONObject> wxLogin(@RequestParam(name = "code", required = true) String code) {
		Result<JSONObject> result = new Result<>();
		JSONObject resJson = new JSONObject();
		//获取access_token和openId
		String url = "https://api.weixin.qq.com/sns/oauth2/access_token?appid=" + LoginConfig.WXAPPID + "&secret=" + LoginConfig.WXSECRET + "&code=" + code + "&grant_type=authorization_code";
		String data = ToolHttpClient.httpGet(url, new HashMap<>(), null);
		...
		String userInfo = ToolHttpClient.httpGet(userInfoUrl, new HashMap<>(), null);
		JSONObject userInfoJson = JSONObject.parseObject(userInfo);
		String nickName = userInfoJson.getString("nickname");
		String sex = userInfoJson.getString("sex");
		String headimgUrl = userInfoJson.getString("headimgurl");
		//判断当前用户是否存在
		/*String unionid="odgGY5yjdckwklSrVOqOXOc5rKQo";
		String nickName="Am";
		String headimgUrl="https://thirdwx.qlogo.cn/mmopen/vi_32/hxdXcxp1wOKuKhiaibDZdrW9rx83DaibRN5UuNgwficsM2AJFBUJnC0p9pDjjCt8Q6OzuSkt3zo1jianAwrjm4K5CTA/132";
		String sex="0";
		String openid="oeynH57DC8lmo19nMF6W9PeyzMQI";*/
		SysUser sysUser = checkUser(unionid);
		//用户存在
		if (oConvertUtils.isNotEmpty(sysUser)) {
			SysUser userNew = new SysUser();
			userNew.setId(sysUser.getId());
	 ...
	 *
	 * @Author: nqr
	 * @Date 2021-03-10 17:07:57
	 */
	private SysUser checkUser(String unionid) {
		LambdaQueryWrapper<SysUser> lambdaQueryWrapper = new LambdaQueryWrapper<>();
		lambdaQueryWrapper.and(x -> x.eq(SysUser::getUnionId, unionid).or().eq(SysUser::getOpenId, unionid));
