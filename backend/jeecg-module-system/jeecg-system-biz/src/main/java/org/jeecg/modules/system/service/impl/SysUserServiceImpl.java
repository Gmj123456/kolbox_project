...
//		}
	 ...
	 * @return JSONObject
	 * @Date 2021-05-24 17:52:51
	 */
	@Override
	public JSONObject currentAmountPayTaxMent(SysUser sysUser, BigDecimal totalAmount) {
		JSONObject jsonObject = new JSONObject();
		String userId = sysUser.getId();
		BigDecimal currentAmount = sysUser.getCurrentAmount();
		// 计算当前余额
		BigDecimal amountNow = currentAmount.subtract(totalAmount);
		if (amountNow.compareTo(BigDecimal.ZERO) < 0) {
		...
		SysUser user = new SysUser();
		user.setId(userId);
		user.setCurrentAmount(amountNow);
	 ...
	 * @param sysUser           用户对象
	 * @param totalAmount       付款金额
	 * @param sellerPromotionId 推广ID
	 * @return JSONObject
	 * @Date 2021-05-24 17:52:51
	 */
	@Override
	public JSONObject currentAmountPayMent(SysUser sysUser, BigDecimal totalAmount, String sellerPromotionId, Integer payType, String remark) {
		JSONObject jsonObject = new JSONObject();
    /*    String userId = sysUser.getId();
        BigDecimal currentAmount = sysUser.getCurrentAmount();
        // 计算当前余额
        ...
        jsonObject.put("user", user);
        // 获取推广需求,修改付款总金额
        StoreSellerPromotion sellerPromotion = storeSellerPromotionService.getById(sellerPromotionId);
        // 已付金额
        BigDecimal paymentAmount = oConvertUtils.isEmpty(sellerPromotion.getPaymentAmount()) ? BigDecimal.ZERO : sellerPromotion.getPaymentAmount();
        // 订单总金额
        BigDecimal totalRmbAmount = oConvertUtils.isEmpty(sellerPromotion.getTotalRmbAmount()) ? BigDecimal.ZERO : sellerPromotion.getTotalRmbAmount();
        ...
        if (payType.equals(PaymentConstant.PAY_TYPE_EARNEST)) {
            sellerPromotionNew.setIntentionAmount(totalAmount);
            ...
            // 设置已付金额
            BigDecimal paymentAmountNew = totalAmount.add(paymentAmount);
            sellerPromotionNew.setPaymentAmount(paymentAmountNew);
            // 判断金额是否正确
            // 意向金
            BigDecimal intentionAmount = oConvertUtils.isEmpty(sellerPromotion.getIntentionAmount()) ? BigDecimal.ZERO : sellerPromotion.getIntentionAmount();
            // 退款金额
            BigDecimal refundAmount = oConvertUtils.isEmpty(sellerPromotion.getRefundAmount()) ? BigDecimal.ZERO : sellerPromotion.getRefundAmount();
            // 总付款金额 = 意向金 + 已付金额 - 退款金额
            BigDecimal paidAmount = paymentAmountNew.add(intentionAmount).subtract(refundAmount);
            if (paidAmount.compareTo(totalRmbAmount) == 0 && totalRmbAmount.compareTo(BigDecimal.ZERO) > 0) {
        ...
        storePromotionPayment.setSellerId(userId);
        storePromotionPayment.setSellerName(sysUser.getUsername());
        storePromotionPayment.setPromId(sellerPromotionId);
        storePromotionPayment.setExtensionCode(sellerPromotion.getExtensionCode());
        storePromotionPayment.setPayAmount(totalAmount);
        storePromotionPayment.setPayRmbAmount(totalAmount);
        ...
        storePromotionPayment.setRemark(remark);
        storePromotionPayment.setPayType(payType);
        jsonObject.put("success", true);
        jsonObject.put("storePromotionPayment", storePromotionPayment);
        jsonObject.put("sellerPromotionNew", sellerPromotionNew);*/
		return jsonObject;
	}

	@Override
	public List<SysUser> getCounselorList() {
		return lambdaQuery().eq(SysUser::getUserType, UserType.CELEBRITY_COUNSELOR.getCode()).eq(SysUser::getDelFlag, 0).eq(SysUser::getIsLeave, 0).list();
	}
	@Override
	@Transactional
	public void addSellerUser(SysUser user, String roles, String selectedParts, StoreEmailPackagePurchase storeEmailPackagePurchase) {
		this.save(user);
		if (oConvertUtils.isNotEmpty(roles)) {
			String[] arr = roles.split(",");
			for (String roleId : arr) {
		...
		if (oConvertUtils.isNotEmpty(storeEmailPackagePurchase.getPackageId())) {
			storeEmailPackagePurchase.setInviteSellerId(user.getId());
