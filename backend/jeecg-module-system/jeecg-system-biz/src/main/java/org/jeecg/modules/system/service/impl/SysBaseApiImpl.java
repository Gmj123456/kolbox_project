...
//			}
	...
	@Override
	public List<LoginUser> getUserByIds(List<String> counselorIds) {
		List<SysUser> sysUsers = userMapper.selectList(new QueryWrapper<SysUser>().in("id", counselorIds));
		return oConvertUtils.entityListToModelList(sysUsers, LoginUser.class);
	}


	@Override
	public void updatePromotionBrand(List<String> productIds, String brandId, String brandName) {
	/*	// 参数验证
		if (productIds == null || productIds.isEmpty()) {
			return;
		}
		if (oConvertUtils.isEmpty(brandId) || oConvertUtils.isEmpty(brandName)) {
			return;
		}
		...
		try {
			// 1. 根据商品查找对应的推广商品
			List<StorePromotionGoods> promotionGoodsList = getPromotionGoodsByProductIds(productIds);
			if (promotionGoodsList.isEmpty()) {
				log.info("未找到相关的推广商品记录");
				return;
			}

			// 2. 根据推广商品查找对应的推广
			List<String> promIds = extractPromIds(promotionGoodsList);
			List<StoreSellerPromotion> promotions = getPromotionsByIds(promIds);
			...
			// 3. 根据推广查找推广下的所有商品
			List<StorePromotionGoods> allPromotionGoods = getAllPromotionGoodsByPromIds(promIds);
			Map<String, KolProduct> allProductsMap = getAllProductsMap(allPromotionGoods);

			// 4. 构造并更新推广的品牌信息
			List<StoreSellerPromotion> updateList = buildUpdatedPromotions(
					promotions, allPromotionGoods, productIds, brandId, brandName, allProductsMap);

			if (!updateList.isEmpty()) {
				storeSellerPromotionService.updateBatchById(updateList);
				log.info("成功更新{}条推广记录，耗时: {}ms", updateList.size(), System.currentTimeMillis() - startTime);
			} else {
				log.info("没有需要更新的推广记录");
			}
		} catch (Exception e) {
			log.error("更新推广品牌信息失败", e);
			throw new RuntimeException("更新推广品牌信息失败: " + e.getMessage());
		}*/
	}
	@Override
	public List<PersonalTagsModel> getPersonalTagsCategory() {
/*		List<StorePersonalTagCategory> personalTagCategories = personalTagCategoryService.getPersonalTagsCategory();
		if (!personalTagCategories.isEmpty()) {
			List<PersonalTagsModel> personalTagsModels = new ArrayList<>();
			for (StorePersonalTagCategory personalTagCategory : personalTagCategories) {
				PersonalTagsModel personalTagsModel = new PersonalTagsModel();
				BeanUtils.copyProperties(personalTagCategory, personalTagsModel);
	...
	@Override
	public LoginUser getCelebrityCounselor(String celebrityCounselorName) {
		LambdaQueryWrapper<SysUser> queryWrapper = new LambdaQueryWrapper<>();
		queryWrapper.eq(SysUser::getRealname, celebrityCounselorName);
	 ...
	 *
	 * @author nqr
	 * @date 2025/3/13 14:00
	 **/
	@Override
	public List<LoginUser> getBusinessUserList() {
		LambdaQueryWrapper<SysUser> queryWrapper = new LambdaQueryWrapper<>();
		...
		List<SysUser> lists = userMapper.selectList(queryWrapper);
		return oConvertUtils.entityListToModelList(lists, LoginUser.class);
	}

	@Override
	public StoreUserContactEmailVo getContactEmailById(String contactEmailId) {
		StoreUserContactEmail userContactEmail = contactEmailService.getById(contactEmailId);
		if (userContactEmail != null) {
			StoreUserContactEmailVo userContactEmailVo = new StoreUserContactEmailVo();
			BeanUtils.copyProperties(userContactEmail, userContactEmailVo);
			return userContactEmailVo;
		}
		return null;
	}

	@Override
	public List<StoreUserContactEmailVo> getContactEmailByUserId(String userId) {
		List<StoreUserContactEmail> userContactEmails = contactEmailService.lambdaQuery()
				.eq(StoreUserContactEmail::getSysUserId, userId)
				.list();
		return oConvertUtils.entityListToModelList(userContactEmails, StoreUserContactEmailVo.class);
	}
}