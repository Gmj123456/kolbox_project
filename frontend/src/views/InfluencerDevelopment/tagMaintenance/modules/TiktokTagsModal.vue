    ...
    <a-spin :spinning="confirmLoading">
      <a-form ref="formRef" :model="model" :rules="validatorRules">
        <template v-if="model.id">
          <a-row style="width: 512px" :gutter="12">
            <a-col :span="24">
              <a-form-item
                label="标签"
                :labelCol="labelCol"
              ...
              >
                <a-input-group compact>
                  <a-select
                    ...
                    placeholder="请选择品牌"
                    style="width: 180px;border-right: 0px;"
                  >
                    <a-select-option
        ...
        <template v-else>
          <a-row :gutter="12" class="platform-type-row">
            <a-col :span="10" >
              <a-form-item name="platformType">
                <a-radio-group
                  button-style="outline"
                  v-model:value="model.platformType"
                  @change="platformTypeChange"
                >
                  <a-radio-button v-for="item in platformTypeList" :key="item.value" :value="item.value">
            ...
            <a-col :span="6">
              <a-form-item name="tagType">
                <a-select v-model:value="model.tagType" placeholder="类型(必填)">
                  <a-select-option v-for="item in tagTypeList" :key="item.value" :value="item.value">
                    {{ item.text }}
                  </a-select-option>
                </a-select>
              </a-form-item>
            ...
            <a-col :span="8">
              <a-form-item name="tagName">
                <a-input v-model:value="model.tagName" placeholder="标签名称(必填)" />
              </a-form-item>
            </a-col>
            <a-col :span="24" v-if="model.platformType == 1 && model.tagType == 2">
              <a-form-item name="videoUrl">
                <a-input v-model:value="model.videoUrl" placeholder="视频链接" />
              </a-form-item>
            </a-col>
          </a-row>
          <a-row :gutter="12">
    ...
    { required: true, message: '请输入视频链接', trigger: 'change' },
    {
      validator: (rule, value, callback) => {
        if (
          ...
          callback(new Error('请输入有效的YouTube视频链接'));
        } else {
          callback();
        }
      },
...
async function queryTagProductAttr() {
  const res = await defHttp.get({url:"/kol/kolTagCategoryRelation/queryByTagProduct", params: {
      tagId: model.value.id,
      productId: model.value.productId,
    }},{isTransformResponse:false})
    if (res.success) {
      const attrIds = res.result.map((item) => item.attributeId);
      handleCategoryList(attrIds);
    }
}
...
function brandIdChange(e) {
  ...
  productList.value = [];
  if (e) {
    if (model.value.productData) {
      productList.value = JSON.parse(model.value.productData)
        .filter((item) => item.brandId == model.value.brandId)
        .map((p) => {
          return {
            id: p.productId,
  ...
  try {
    ...
    if (res) {
      ...
      if (productList.value.length === 1) {
        model.value.productId = productList.value[0].id;
      }
      if (model.value.id) {
        getProductData();
      }
...
async function getProductData() {
  try {
    const res = await queryTagProductByTagIdApi({ tagId: model.value.id });
    if (res) {
      model.value.productId = res? res.split(',') : '';
    }
  } catch (error) {
...
function platformTypeChange() {
  model.value.tagType = undefined;
  setTypeList(model.value.platformType);
}

function setTypeList(e) {
  ...
  if (matchedGroup && matchedGroup.value) {
    tagTypeList.value = matchedGroup.value
      .split(',')
      .map((j) => tagTypeAllList.value.find((item) => item.value == j.trim()))
...
async function initTagTypeAndGroup() {
  try {
    const [tagTypeRes, tagGroupRes, platformTypeRes] = await Promise.all([
      getDictItems('tag_type'),
      getDictItems('tag_group'),
      getDictItems('platform_type'),
    ]);
    if (tagTypeRes && tagGroupRes && platformTypeRes) {
      tagTypeAllList.value = tagTypeRes;
...
function edit(record) {
  ...
  model.value = { ...record };
  if (record.id) {
    model.value.tagType = String(record.tagType);
    model.value.platformType = String(record.platformType);
    brandList.value = record.productData
    ? Array.from(
        new Map(
          JSON.parse(record.productData).map((item) => [
            item.brandId,
            { id: item.brandId, brandName: item.brandName },
          ])
        ).values()
      )
    : [];
    if (record.productData && JSON.parse(record.productData).length == 1) {

      model.value.brandId = JSON.parse(record.productData)[0].brandId;
      productList.value = JSON.parse(model.value.productData)
        .filter((item) => item.brandId == model.value.brandId)
        .map((p) => {
          return {
      ...
      model.value.productId = JSON.parse(record.productData)[0].productId;
    } else {
...
async function executeSubmit() {
  ...
  socialMediaCategoryList.value.forEach((item) => {
    if (item.filterCategoryName && item.oldSelectedIds) {
      item.selectedIds = [...item.oldSelectedIds];
    }
  });
  confirmLoading.value = true;
  try {
    const { platformType, tagType, productId, tagName, id } = model.value;
    const params = {
      platformType,
      tagType,
      tagName,
    ...
    if (model.value.platformType == 1 && model.value.tagType == 2) {
      params.videoUrl = model.value.videoUrl;
    }
    if (model.value.id) {
      params.id = id;
    }
    ...
    const influencerType = params.dataList.find((item) => item.typeId === '20250513000003');
    if (!influencerType || influencerType.list.length === 0) {
      createMessage.error('至少选择一个达人类型');
      confirmLoading.value = false;
      return;
    }
    const res = model.value.id ? await editTagApi(params) : await addTagApi(params);
    if (res.success) {
