  ...
  >
    <a-spin :spinning="confirmLoading">
      <vxe-table :data="dataSourceList" size="small" height="400">
        ...
        <vxe-table-column field="brandCategory" title="品牌类目"></vxe-table-column>
        <vxe-table-column field="email" title="邮箱"></vxe-table-column> -->
      
        <vxe-table-column field="errorMsg" title="失败信息">
          
          <template #default="{ row }">
          {{ row.errorMsg }}
            <!-- <div v-for="(error, index) in row.errorMsg" :key="index">{{ error }}</div> -->
          </template>
        </vxe-table-column>
      </vxe-table>
    </a-spin>
...
function show(data) {
  visible.value = true;
  dataSourceList.value = data.map((item,i) => {
    return {
      id: i + 1,
      ...item
    }
