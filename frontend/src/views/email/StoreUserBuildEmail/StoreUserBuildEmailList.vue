<template>
  <a-card :bordered="false">
      ...
      <a-row  :gutter="24">
        ...
        <a-col :xl="6" :lg="7" :md="8" :sm="24">
          ...
          >
            ...
            <a-button
              type="primary"
              v-if="hasPermission('user:editreplacementemail')"
              @click="replaceEmail"
        ...
        <template #expandedRowRender="{ record }">
          ...
          >
            <template #bodyCell="{ column, record: innerRecord, text }">
              ...
              <template v-else-if="column.dataIndex === 'signatureContent'">
                <EllipsisTooltip :text="text" :lines="1"></EllipsisTooltip>
              </template>
        ...
        <template #bodyCell="{ column, record, text }">
          ...
          <template v-else-if="column.dataIndex === 'action'">
            <a-tooltip title="邮箱授权">
              <a :disabled="record.isAuthorized == 1" @click="handleEmailAuth(record)">
                <span
    ...
    <!-- 表单区域 -->
    <storeUserContactEmail-modal
      ref="modalFormRef"
      @ok="modalFormOk"
    ></storeUserContactEmail-modal>
    <ReplaceContachEmailModel
      ref="ReplaceContachEmailRef"
      @ok="modalFormOk"
    ></ReplaceContachEmailModel>
...
import { SearchOutlined, ReloadOutlined, PlusOutlined, RetweetOutlined, FormOutlined,PlusSquareOutlined,MinusSquareOutlined   } from '@ant-design/icons-vue';
import { defHttp } from '/@/utils/http/axios';
import { useMessage } from '/@/hooks/web/useMessage';
import { useTable } from '/@/components/useSTable/useSTable';
import StoreUserContactEmailModal from "./modules/StoreUserContactEmailModal.vue";
import ReplaceContachEmailModel from "./modules/ReplaceContachEmailModel.vue";
...
const {
 ...
 function userBuildEmailAfterFetchFn(params,res) {
  // dataSource.value
...
async function handleDeleteSignature(record, innerRecord) {
  ...
  try {
    const res = await defHttp.delete({ url: url.innerTableDelete, data: { id: innerRecord.id } },{ joinParamsToUrl: true,isTransformResponse:false });
    if (res.success) {
...
async function handleDelete(id) {
  ...
  try {
    const res = await defHttp.delete({ url: url.delete, data: {type:0, id } },{ joinParamsToUrl: true,isTransformResponse:false });
    if (res.success) {
      createMessage.success(res.message);
