...
function onBrandChange(value) {
  ...
  if (brandProductList.value.length == 1) {
    model.productId = brandProductList.value[0].id;
    initEmailTemplate();
  }
}
...
async function initEmailTemplate() {
  ...
  if (model.sendType == 0) {
    params.platformType = model.platformType;
  }
  const res = await defHttp.get({ 
...
async function show(record) {
  ...
  brandProductList.value = productList.value.filter(
    (item) => item.brandId == model.brandId
  );
  onProductChange(record.productId);
}
...
async function handleOk() {
  try {
    ...
    const params = {
      ...
      brandName: brandList.value.find((item) => item.id == model.brandId)
        ?.brandName,
