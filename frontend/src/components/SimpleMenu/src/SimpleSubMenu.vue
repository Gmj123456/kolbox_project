<template>
  <MenuItem :name="item.path" v-if="!menuHasChildren(item) && getShowMenu" v-bind="$props" :class="getLevelClass">
    <Icon v-if="getIcon" :icon="getIcon" :size="collapse?24:14" />
    ...
    <a
      ...
      @click.prevent
    >
      <span :class="['ml-2', `${prefixCls}-sub-title`]" style="font-size: 14px;">
        {{ getI18nName }}
      </span>
  ...
  >
    <template #title>
      ...
      <span v-show="getShowSubTitle" :class="['ml-2', `${prefixCls}-sub-title`]" style="font-size: 14px;">
        {{ getI18nName }}
      </span>
